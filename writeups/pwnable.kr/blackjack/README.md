...
2. The task was about finding a bug in the code which leads to win blackjack game.
Source code of the gamme running on the server:
```c
...
3. The issue was in function `betting`, you can assign a big value to `bet` variable because on the second try, there is no check `if (bet > cash)` performed.
```
int betting() //Asks user amount to bet
{
 printf("\n\nEnter Bet: $");
